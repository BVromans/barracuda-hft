...
**Focus on Two Key Files**
	...
	* `src/index.ts`
		Where the main flow of the system starts.

	* `src/rujira.ts`
		The heart of the system, when the main implementaiton is done.
	...
	* `integration/index.test.ts`
		...
		* If an RPC connection / internet call fails, wait a few seconds and retry.
